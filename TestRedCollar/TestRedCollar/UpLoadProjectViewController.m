...
@interface UpLoadProjectViewController ()

@end

@implementation UpLoadProjectViewController
{
    NSData *dataPhoto;
...
{
    if (miIndex == 1) {
        self.mFilePath = sender.mLocalPath;
        UIImage *image = [UIImage imageWithContentsOfFile:sender.mLocalPath];
        [mSelectBtn setImage:image forState:UIControlStateNormal];
    }
}
...
//    mScrollView.scrollEnabled = NO;
    ...
    textLabel.text = @"选择一张图片后，你可以将你的设计说明和描述，进行填写。";
    ...
    selectBtn.frame = CGRectMake(20+100*1, iHeight+10, 80, 80);
