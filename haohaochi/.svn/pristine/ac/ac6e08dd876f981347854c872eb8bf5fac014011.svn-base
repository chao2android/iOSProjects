...
- (id)initWithItem:(NSDictionary *)item
{
    self = [super init];
    if (self) {
        _videoPath = [self GetFormatPath:[item objectForKey:@"videoPath"]];
        _videoThumbnailPath = [self GetFormatPath:[item objectForKey:@"videoThumbnailPath"]];
        ...
        NSLog(@"DWUploadItem:%@", _uploadContext);
    }
...
- (NSString *)GetFormatPath:(NSString *)path {
    ...
    NSString *newpath = [kDocumentPath stringByAppendingPathComponent:@"MyVideos/"];
...
- (id)initWithPath:(NSString *)filename
{
    ...
    if (self) {
        ...
        _items = items;
        NSLog(@"load %ld %@", (long)[_items count], _items);
    }
...
- (BOOL)writeToPlistFile:(NSString*)filename
{
    ...
    NSLog(@"write %ld %@ to %@", (long)array.count, array, path);
...
- (NSArray *)readFromPlistFile:(NSString*)filename
{
    ...
    NSLog(@"load: %@ count %ld items: %@", path, (long)[array count], array);
