...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateCellProgress:) name:kMsg_UploadProgress object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateUploadStatus:) name:kMsg_UploadStatus object:nil];
        [self createView];
    }
    return self;
}
...
-(void)setupCell:(DWUploadItem *)item
{
    ...
    NSLog(@"statusTitle: %@ imageName: %@", statusTitle, imageName);
...
- (void)updateCellProgress:(NSNotification *)noti
{
    DWUploadItem *item = [noti.userInfo objectForKey:@"item"];
    if (item != self.mItem) {
        return;
    }
    [self.progressView setProgress:[item videoUploadProgress]];
...
- (void)updateUploadStatus:(NSNotification *)noti
{
    DWUploadItem *item = [noti.userInfo objectForKey:@"item"];
    if (item != self.mItem) {
        return;
