...
@interface HomeViewController ()<SelectTabBarDelegate, LocationManagerDelegate> {
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(OnQuitClick) name:kMsg_Logout object:nil];
    ...
    [LocationManager Share].delegate = self;
    [LocationManager Share].mbGetName = YES;
    [[LocationManager Share] StartLocation];
}

- (void)didUpdateCityName:(LocationManager *)locationManager {
    if (mCityView) {
        mCityView.mCity = [LocationManager Share].mCity;
        [mCityView RefreshView];
    }
}
