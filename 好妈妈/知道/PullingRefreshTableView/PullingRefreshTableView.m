...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
		...
		self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"backgound.png"]];
    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            if (!self.isAtTop) {    //footer
                _arrow.hidden = YES;
                _stateLabel.text = NSLocalizedString(@"", @"");
            }
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        // Initialization code
        if ([self.tabTag isEqualToString:@"10002"]) {
            
            UILabel* lab=[[UILabel alloc]initWithFrame:CGRectMake(0, 0, 210, 200)];
            [self addSubview: lab];
            [lab release];
        }
...
- (void)flashMessage:(NSString *)msg{
    //Show message
    __block CGRect rect = CGRectMake(10, self.contentOffset.y - 20, self.bounds.size.width-20, 20);
    ...
    if (_msgLabel == nil) {
        ...
        _msgLabel.backgroundColor = [UIColor colorWithRed:0/255.0 green:147/255.0 blue:198/255.0 alpha:1.0];
