...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
		...
		mTimeLabel.font = [UIFont systemFontOfSize:12.0f];
...
//		mTimeLabel.shadowOffset = CGSizeMake(0.0f, 1.0f);
        ...
        NSString *lastTime = [self GetFormatDateTime];
		if (lastTime) {
            mTimeLabel.text = [NSString stringWithFormat:@"上次更新时间: %@", lastTime];
		}
		else {
			[self setCurrentDate];
		}
		...
		mStatusLabel.font = [UIFont boldSystemFontOfSize:13.0f];
...
- (void)setCurrentDate {
    ...
    NSString *lasttime = [formatter stringFromDate:[NSDate date]];
	[[NSUserDefaults standardUserDefaults] setObject:lasttime forKey:@"EGORefreshTableView_LastRefresh"];
...
- (NSString *)GetFormatDateTime {
    NSString *lasttime = [[NSUserDefaults standardUserDefaults] objectForKey:@"EGORefreshTableView_LastRefresh"];
    if (lasttime) {
        ...
        NSString *lasttimestr = [formatter2 stringFromDate:lastdate];
        NSString *curtimestr = [formatter2 stringFromDate:[NSDate date]];
        if ([lasttimestr isEqualToString:curtimestr]) {
            lasttime = [lasttime stringByReplacingOccurrencesOfString:lasttimestr withString:@"今日"];
        }
        return lasttime;
    }
    return nil;
