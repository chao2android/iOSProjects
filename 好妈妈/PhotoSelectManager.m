...
@synthesize mRootCtrl, mDefaultName, delegate, OnPhotoSelect, mLocalPath, miMinWidth, mbEdit,popoverController;
...
//    [imageCtrl release];
    ...
    if (ISIPAD) {
        ...
        [ popoverController presentPopoverFromRect:CGRectMake(200, 700, 60, 200) inView:mRootCtrl.view permittedArrowDirections:UIPopoverArrowDirectionDown animated:YES];
    }else{
...
didFinishPickingMediaWithInfo: (NSDictionary *) info {
    ...
    if ([[info objectForKey:UIImagePickerControllerMediaType] isEqualToString:(NSString*)kUTTypeImage]) {
        ...
        if (iWidth>miMinWidth && iHeight > miMinWidth) {
            iWidth = miMinWidth;
            iHeight = image.size.height*iWidth/image.size.width;
            if (iHeight<miMinWidth) {
                iHeight = miMinWidth;
