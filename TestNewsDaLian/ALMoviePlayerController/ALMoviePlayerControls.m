...
- (void)setup {
   ...
   // [self addSubview:_topBar];
    ...
    //自定义滑块
    UIImage *thumbImage = [UIImage imageNamed:@"14.png"];
    [_durationSlider setThumbImage:thumbImage forState:UIControlStateHighlighted];
    [_durationSlider setThumbImage:thumbImage forState:UIControlStateNormal];
    ...
    if (_style == ALMoviePlayerControlsStyleFullscreen || (_style == ALMoviePlayerControlsStyleDefault && _moviePlayer.isFullscreen)) {
        ...
        //[_fullscreenButton setBackgroundColor:[UIColor orangeColor]];
        ...
        //_bottomBar.backgroundColor = [UIColor purpleColor];
        ...
        [_scaleButton addTarget:self action:@selector(fullscreenPressed:) forControlEvents:UIControlEventTouchUpInside];
        [_scaleButton setBackgroundColor: [UIColor greenColor]];
       ...
       // [_seekForwardButton setImage:[UIImage imageNamed:@"movieForward.png"] forState:UIControlStateNormal];
        ...
        //[_fullscreenButton setBackgroundColor:[UIColor redColor]];
        [_fullscreenButton addTarget:self action:@selector(fullscreenPressed:) forControlEvents:UIControlEventTouchUpInside];
        _fullscreenButton.delegate = self;
        [_bottomBar addSubview:_fullscreenButton];
    }
    ...
    [_playPauseButton setImage:[UIImage imageNamed:@"kaishi.png"] forState:UIControlStateNormal];
    [_playPauseButton setImage:[UIImage imageNamed:@"zanting.png"] forState:UIControlStateSelected];
...
//        self.topBar.frame = CGRectMake(0, self.frame.size.height - self.barHeight, self.frame.size.width, self.barHeight);
//        self.fullscreenButton.frame = CGRectMake(self.topBar.frame.size.width - paddingFromBezel - scaleWidth, self.barHeight/2 - scaleHeight/2, scaleWidth, scaleHeight);
...
//        self.playPauseButton.frame = CGRectMake(self.bottomBar.frame.size.width/2 - playWidth/2, self.barHeight/2 - playHeight/2, playWidth, playHeight);
        ...
        self.bottomBar.frame = CGRectMake(0, self.frame.size.height - 40, self.frame.size.width, 40);
        self.playPauseButton.frame = CGRectMake(0, 0, 67, 40);
        self.timeElapsedLabel.frame = CGRectMake(self.playPauseButton.frame.origin.x + self.playPauseButton.frame.size.width + paddingBetweenButtons, 0, labelWidth, self.barHeight-28);
        
        //right side of bottom bar
        CGFloat fullscreenWidth = 28.f;
        CGFloat fullscreenHeight = fullscreenWidth;
        self.fullscreenButton.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - fullscreenWidth, self.barHeight/2 - fullscreenHeight/2-15, fullscreenWidth, fullscreenHeight);
        ...
        self.timeRemainingLabel.frame = CGRectMake(self.airplayView.frame.origin.x - paddingBetweenButtons - labelWidth + 60, 0, labelWidth + 20, self.barHeight-28);
...
//        self.airplayView.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - airplayWidth, self.barHeight/2 - airplayHeight/2, airplayWidth, airplayHeight);
        ...
        self.airplayView.frame = CGRectMake(self.fullscreenButton.frame.origin.x - paddingBetweenButtons - airplayWidth, self.barHeight/2 - airplayHeight/2, airplayWidth, airplayHeight);
        self.timeRemainingLabel.frame = CGRectMake(self.airplayView.frame.origin.x - paddingBetweenButtons - labelWidth+ 50, 0, labelWidth + 20, self.barHeight-28);
    }
    ...
    self.durationSlider.frame = CGRectMake(timeElapsedX + self.timeElapsedLabel.frame.size.width + paddingBetweenLabelsAndSlider, self.barHeight/2 - sliderHeight/2, sliderWidth, sliderHeight-28);
