...
- (id)initWithFrame:(CGRect)frame withImageArray:(NSArray*)array
{
    ...
    if (self) {
        ...
        for (int i = 0; i<array.count; i++) {
            ...
            NSString *imagename = [NSString stringWithFormat:@"%@.png",[array objectAtIndex:i]];
            NSString *imagename2 = [NSString stringWithFormat:@"%@_pressed.png",[array objectAtIndex:i]];
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            button.frame = CGRectMake(iLeft, 0, iWidth, 49);
...
- (void)selectNumber:(int)number {
    ...
    //添加背景红色小球
    imageView = [[UIImageView alloc]initWithFrame:CGRectMake(self.selectBtn.frame.origin.x+4, 0,self.selectBtn.frame.size.width-9, 45)];
...
- (void)tabBtnAction:(UIButton*)btn {
    ...
    if (self.selectBtn.tag != btn.tag) {
        ...
        imageView.frame = CGRectMake(self.selectBtn.frame.origin.x+4, imageView.frame.origin.y, imageView.frame.size.width, imageView.frame.size.height);
