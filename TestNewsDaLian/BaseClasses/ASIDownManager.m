...
- (void)PostHttpRequest:(NSString *)urlString :(NSDictionary *)dict files:(NSDictionary *)files {
   ...
   NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:kUrlStr,urlString]];
    
    mRequest = [[ASIFormDataRequest alloc] initWithURL:url];

    [mRequest addRequestHeader:@"Content-Type" value:@"application/json; encoding=utf-8"];
    [mRequest addRequestHeader:@"Accept" value:@"application/json"];
    [mRequest setRequestMethod:@"POST"];
    [mRequest setTimeOutSeconds:100];
    [mRequest setPostBody:tempJsonData];
...
// add by dxy
- (void)postHttpRequest3:(NSString *)urlString {
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:kUrlStr,urlString]];
    mRequest = [[ASIFormDataRequest alloc] initWithURL:url];
    [mRequest addRequestHeader:@"Content-Type" value:@"application/json; encoding=utf-8"];
    [mRequest addRequestHeader:@"Accept" value:@"application/json"];
