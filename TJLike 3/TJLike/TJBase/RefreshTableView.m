...
@synthesize mDelegate, mbMoreHidden, mbRefreshHidden, separatorColor, separatorStyle, miBotOffset, contentSize, contentOffset;
...
{
    if (mDelegate && [mDelegate respondsToSelector:@selector(numberOfSectionsInTableView:)]) {
...
{
    if (mDelegate && [mDelegate respondsToSelector:@selector(tableView:numberOfRowsInSection:)]) {
        return [mDelegate tableView:tableView numberOfRowsInSection:section];
    }
    return 0;
}
...
{
    if (mDelegate && [mDelegate respondsToSelector:@selector(tableView:heightForRowAtIndexPath:)]) {
        return [mDelegate tableView:tableView heightForRowAtIndexPath:indexPath];
    }
    return 40;
}
...
{
    if (mDelegate && [mDelegate respondsToSelector:@selector(tableView:cellForRowAtIndexPath:)]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (mDelegate && [mDelegate respondsToSelector:@selector(tableView:didSelectRowAtIndexPath:)]) {
        [mDelegate tableView:tableView didSelectRowAtIndexPath:indexPath];
    }
}

- (BOOL)CanRefresh {
    if (mDelegate && [mDelegate respondsToSelector:@selector(CanRefreshTableView:)]) {
        return [mDelegate CanRefreshTableView:self];
    }
    return NO;
}
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
	if (scrollView.contentOffset.y <= - 65.0f && [self CanRefresh]) {
        ...
        if (mDelegate && [mDelegate respondsToSelector:@selector(ReloadList:)]) {
            return [mDelegate ReloadList:self];
        }
	}
}
...
- (void)OnLoadMoreClick {
    if (!mLoadMore.hidden && [self CanRefresh]) {
        ...
        if (mDelegate && [mDelegate respondsToSelector:@selector(LoadMoreList:)]) {
            return [mDelegate LoadMoreList:self];
        }
    }
}
