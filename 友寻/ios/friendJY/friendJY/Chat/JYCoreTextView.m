    ...
    if (_IDRanges.count> 0) {
        for (NSValue *temp in _IDRanges) {
            if (self.IDColor == nil) {
                ...
                [self.attributedString addAttributes:@{(NSString *)kCTForegroundColorAttributeName:(id)self.IDColor.CGColor} range:temp.rangeValue];
            }
            
        }
    }
    
    CGSize size = [self sizeForDrawAttributedString:_attributedString width:self.maxWidth];
    [self setBounds:CGRectMake(0, 0, size.width, size.height+2.0f)];
...
{
//    CTFontRef fontRef = CTFontCreateWithName((CFStringRef)[UIFont systemFontOfSize:self.fontSize].fontName, self.fontSize, NULL);
    CTFontRef fontRef = CTFontCreateWithName((CFStringRef)@"Helvetica-Light", 14.0f, NULL);
    CGFloat widthValue = -1.0;
    CFNumberRef strokeWidth = CFNumberCreate(NULL,kCFNumberFloatType,&widthValue);
    CGColorRef theCGColorRef = self.textColor.CGColor;
    NSDictionary* attrs = [[NSDictionary alloc] initWithObjectsAndKeys:
                           ...
                           (id)strokeWidth, kCTStrokeWidthAttributeName, nil];
