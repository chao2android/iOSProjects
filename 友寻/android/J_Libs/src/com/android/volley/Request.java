...
public abstract class Request<T> implements Comparable<Request<T>> {
	...
	public Request(int method, String url, Response.ErrorListener listener) {
		...
		} else if (TextUtils.isEmpty(Uri.parse(url).getHost())) {
			mDefaultTrafficStatsTag = 0;
		} else {
	 ...
	 * @param listener
	 */
	public Request(int method, String url, HashMap<String, String> params, Response.ErrorListener listener) {
		mMethod = method;
		mUrl = url;
		mParams = params;
		mErrorListener = listener;
	...
	/**
	 * TODO Returns a Map of parameters to be used for a POST or PUT request.
	 * Can throw {@link AuthFailureError} as authentication may be required to
	 * provide these values.
	 * 
	 * <p>
	 ...
	 * @throws AuthFailureError in the event of auth failure
	 * 
	 * @deprecated 只有getparams返回的map值，才可以启动post请求，否则使用的是get请求
	 */
	@Deprecated
	protected Map<String, String> getParams() throws AuthFailureError {
		// 解析url从中获取传递的参数
		if (getMethod() == Method.POST && mParams.size() > 0) {
			return mParams;
		}
