...
public class InvitationGroupProtocol extends AbsPushProtocol {
	 ...
	 * 1 0 0 a . j p g ? w h = 1 0 0 x 1 0 0 " , "title":"咯去破坏","logo":
	 * "http:\/\/p.friendly.dev\/09\/19\/07d80dfe7279d4c584cd33fb8aec3763\/142050_1
	 * 0 0 i . j p g ? w h = 1 0 0 x 1 0 0
	...
	public SocketProtocol getResponseObj(JSONObject json) throws JSONException {
		...
		// if (false && SharedPreUtil.getLoginInfo().nickName.equals(joinNick))
		// {
		...
		// bean.showIcon = JsonUtil.getJsonString(json,
		// UtilRequest.FORM_AVATARS);
		// return this;
		// } else {
		...
		if (SharedPreUtil.getLoginInfo().nickName.equals(joinNick)) {
			notificationMsg = bean.contentText = "你已经加入了群组，可以开始聊天啦";
		} else {
			bean.contentText = joinNick + "已经加入了 " + bean.nick;
			notificationMsg = joinNick + "已经加入了群组";
		}
		...
		item.count = 0;
		int lc_msg_id = saveMsgData(bean, notificationMsg);
	...
	private int saveMsgData(PushMsgInfo bean, String string) {
		...
		cursor.close();
		// 更新该用户的最后一条聊天信息
		helper.updateLastMessage(ci.getContactId(), string, ci.getStatus(), ci.getTimeStamp(), ci.getChatType(), ci.getGroupId());
		return ci.getId();
	}
}
