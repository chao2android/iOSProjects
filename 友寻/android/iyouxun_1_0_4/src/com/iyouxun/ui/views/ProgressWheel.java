...
public class ProgressWheel extends View {
	...
	private void setupBounds() {
		...
		circleBounds = new RectF(paddingLeft + barWidth, paddingTop + barWidth, width - paddingRight - barWidth, height
	...
	private void parseAttributes(TypedArray a) {
		barWidth = (int) a.getDimension(R.styleable.ProgressWheel_m_barWidth, barWidth);

		rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_m_rimWidth, rimWidth);

		spinSpeed = (int) a.getDimension(R.styleable.ProgressWheel_m_spinSpeed, spinSpeed);

		delayMillis = a.getInteger(R.styleable.ProgressWheel_m_delayMillis, delayMillis);
		if (delayMillis < 0) {
			delayMillis = 0;
		}

		barColor = a.getColor(R.styleable.ProgressWheel_m_barColor, barColor);

		barLength = (int) a.getDimension(R.styleable.ProgressWheel_m_barLength, barLength);

		textSize = (int) a.getDimension(R.styleable.ProgressWheel_m_textSize, textSize);

		textColor = a.getColor(R.styleable.ProgressWheel_m_textColor, textColor);

		// if the text is empty , so ignore it
		if (a.hasValue(R.styleable.ProgressWheel_m_text)) {
			setText(a.getString(R.styleable.ProgressWheel_m_text));
		}

		rimColor = a.getColor(R.styleable.ProgressWheel_m_rimColor, rimColor);

		circleColor = a.getColor(R.styleable.ProgressWheel_m_circleColor, circleColor);

		contourColor = a.getColor(R.styleable.ProgressWheel_m_contourColor, contourColor);
		contourSize = a.getDimension(R.styleable.ProgressWheel_m_contourSize, contourSize);

		mProgress = a.getInteger(R.styleable.ProgressWheel_m_progress, mProgress);
	...
	private final static String TAG = "PWHELL_p";
	...
	protected void onDraw(Canvas canvas) {
		...
		// Draw the bar
		if (isSpinning) {
			canvas.drawArc(circleBounds, mProgress - 90, barLength, false, barPaint);
		} else {
		...
		splitText = new String[] { "" + mProgress };

		if (!"0".equals(splitText[0]))
