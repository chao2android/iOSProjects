...
public class ProfileGroupActivity extends CommTitleActivity {
	...
	protected void initViews() {
		...
		// 添加底部提示信息
		View footerView = View.inflate(mContext, R.layout.group_manage_warm_layer, null);
		group_warm_box = (LinearLayout) footerView.findViewById(R.id.group_warm_box);
		profile_groups_lv.addFooterView(footerView, null, false);
		...
		profile_groups_lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				GroupsInfoBean bean = datas.get(position);
				int currentStatus = -1;
				if (bean.show == 1) {
					currentStatus = 0;
				} else {
