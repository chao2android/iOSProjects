...
public class ProfileViewActivity extends CommTitleActivity {
	...
	private TextView profile_confirm_single;// 确认婚姻状态
	private RelativeLayout profile_confirm_marriage_box;// 已经确认婚姻状态
	private TextView profile_confirm_marriage_count;
	...
	private TextView profile_right_icon1;// 共同好友右侧按钮
	...
	protected void initViews() {
		...
		profile_confirm_single = (TextView) findViewById(R.id.profile_confirm_single);
		profile_confirm_marriage_box = (RelativeLayout) findViewById(R.id.profile_confirm_marriage_box);
		profile_confirm_marriage_count = (TextView) findViewById(R.id.profile_confirm_marriage_count);
		...
		profile_confirm_single.setOnClickListener(listener);
		profile_tag_toggle.setOnClickListener(listener);
		profile_out_single_button.setOnClickListener(listener);
		profile_confirm_marriage_box.setOnClickListener(listener);
	...
	private final OnClickListener listener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.profile_avatar:
				...
				break;
			case R.id.profile_confirm_marriage_box:
				// 取消确认单身
			...
			case R.id.profile_add_friend_button:
				// 加为好友
				UtilRequest.friendsSentReq(J_Cache.sLoginUser.uid, currentUid, mHandler);
				break;
			...
			case R.id.profile_right_icon1:
				...
				break;
			case R.id.profile_confirm_single:
				// 确认单身
	...
	private void setContent() {
		...
		// 情感状况
		profile_marriage.setText(ProfileUtils.getMarriage(currentUserInfo.marriage));
		// 个人签名
		...
		if (currentUserInfo.marriage == 1) {
			// 生日星座
			if (currentUserInfo.star > 0 && currentUserInfo.birthpet > 0) {
				totalShow++;
			...
			if (currentUserInfo.marriage == 1) {
				...
				if (currentUserInfo.isFriend == 1) {
					...
					if (currentUserInfo.isLonelyConfirm == 0) {
						// 未确认过
						profile_confirm_single.setVisibility(View.VISIBLE);
						profile_confirm_marriage_box.setVisibility(View.GONE);
						profile_confirm_single.setText("我确认TA是单身");
					} else {
						// 确认过
						profile_confirm_single.setVisibility(View.GONE);
						profile_confirm_marriage_box.setVisibility(View.VISIBLE);
						profile_confirm_marriage_count.setText(currentUserInfo.lonelyConfirm + "");
					}
				} else {
					// 非好友
					profile_confirm_single.setVisibility(View.VISIBLE);
					profile_confirm_single.setText("帮TA脱单");
				}
				// 单身状态，显示“帮TA脱单”按钮
	...
	private void setSameFriends() {
		if (sameFriendsCount > 0 && sameFriends.size() > 0) {
			...
			for (int i = 0; i < sameFriends.size(); i++) {
				...
				userStr.setSpan(new ClickableSpan() {
					@Override
					...
					public void updateDrawState(TextPaint ds) {
						...
						ds.setColor(getResources().getColor(R.color.text_normal_blue));
						ds.setUnderlineText(false);
						ds.clearShadowLayer();
						ds.bgColor = mContext.getResources().getColor(R.color.transparent);
					}
				}, 0, userNick.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			...
			if (sameFriendsCount > 1) {
				...
				countStr.setSpan(new ClickableSpan() {
					@Override
					public void onClick(View widget) {
						Intent sameIntent = new Intent(mContext, ProfileSameFriendsActivity.class);
						sameIntent.putExtra("uid", currentUid);
						sameIntent.putExtra("userInfo", currentUserInfo);
				...
				}, 0, countTxt.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
				ssBuilder.append(countStr);
				ssBuilder.append("人");
			}
			profile_same_friends.setText(ssBuilder);
			profile_same_friends.setMovementMethod(LinkMovementMethod.getInstance());
		} else {
			SpannableStringBuilder ssBuilder = new SpannableStringBuilder();
			String noFriendStr = "还没有共同好友哦";
			SpannableString noFriendTxt = new SpannableString(noFriendStr);
			noFriendTxt.setSpan(new ClickableSpan() {
				@Override
				public void onClick(View widget) {
					Intent sameFriendIntent = new Intent(mContext, ProfileSameFriendsActivity.class);
					sameFriendIntent.putExtra("type", 1);
			...
			}, 0, noFriendStr.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
