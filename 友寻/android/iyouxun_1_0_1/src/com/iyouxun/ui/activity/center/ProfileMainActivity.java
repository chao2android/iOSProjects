...
public class ProfileMainActivity extends CommTitleActivity {
	...
	private TextView profile_confirm_single;// 好友认证数量显示
	...
	private void getOneUserDynamicList() {
		UtilRequest.getOneUserDynamicList(J_Cache.sLoginUser.uid, 0, 1, mHandler);
	}
	...
	private void setContent() {
		...
		// 婚姻状况
		profile_marriage.setText(ProfileUtils.getMarriage(J_Cache.sLoginUser.marriage));
		if (J_Cache.sLoginUser.marriage == 1) {
			// 单身
			profile_confirm_single.setVisibility(View.VISIBLE);
			profile_confirm_single.setText(J_Cache.sLoginUser.lonelyConfirm + "好友认证");
			// 显示“求脱单按钮”
			titleRightButton.setVisibility(View.VISIBLE);
			// 显示“邀请好友帮我认证”
