...
public class AddFriendsActivity extends CommTitleActivity {
	 ...
	 * @Fields textId : 展示的文本
	 */
	private final int[] textId = { R.string.add_contact_friends, R.string.add_wechat_friends, R.string.add_weibo_friends,
			R.string.add_qq_friends };
	/**
	 * @Fields icnId : 展示的图标
	 */
	private final int[] icnId = { R.drawable.icn_contact, R.drawable.icn_wechat, R.drawable.icn_sina_weibo, R.drawable.icn_qq };
			...
			case 1:// 添加微信好友
				...
				new WeChatShareDialog(mContext, R.style.dialog).setCallBack(new DialogUtils.OnSelectCallBack() {
					...
					public void onCallBack(String value1, String value2, String value3) {
						if (value1.equals("1")) {
							// 微信好友
							addFriendByPlatform(Wechat.NAME);
						} else if (value1.equals("2")) {
							// 微信朋友圈
							addFriendByPlatform(WechatMoments.NAME);
						}
					}
				}).show();
				break;
			...
			if (arg0.getName().equals(SinaWeibo.NAME)) {
				if ((arg1 & Platform.CUSTOMER_ACTION_MASK) == GET_SINA_WEIBO_FRIENDS_ACTION) {
					...
					if (arrayList.size() <= 0) {
						...
						if (sinaDataObject.length() > 0) {
							mHandler.post(new Runnable() {

								@Override
								public void run() {// 上传新浪微博好友数据
									UtilRequest.uploadSinaFriends(sinaDataObject.toString(), mContext);
								}
							});
						}
