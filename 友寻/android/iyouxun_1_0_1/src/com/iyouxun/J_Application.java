...
public class J_Application extends FrontiaApplication {
	...
	public void onCreate() {
		...
		if (getAppUid() > 0) {
			...
			config.USER_ID = "";
			config.LOG_ENABLE = false;// 是否打印日志信息
	...
	private void initLocations() {
		J_LocationManager j_LocationManager = J_LocationManager.getInstance();

		j_LocationManager.init(new J_LocationCallback() {
			@Override
			public void onLocationSuccess(J_LocationPoint point) {
				DLog.i("J_Location", "application:" + point.getLongitude() + "," + point.getLatitude());
