...
public class NewsListInfoAdapter extends BaseAdapter {
	...
	/** 是否显示上传通讯录引导 */
	private boolean isGuideShow1 = false;
	private boolean isGuideShow2 = false;
	private boolean isGuideShow3 = false;

	private final long CURRENT_UID;

	public NewsListInfoAdapter(Context mContext, long CURRENT_UID) {
		this.mContext = mContext;
		this.CURRENT_UID = CURRENT_UID;

		if (CURRENT_UID == 0 && SharedPreUtil.getLoginInfo().callno_upload == 0) {
			isGuideShow1 = true;
			isGuideShow2 = true;
			isGuideShow3 = true;
		}
	...
	public void setGuideShowStatus(boolean isCanShow) {
		if (isCanShow) {
			if (CURRENT_UID == 0 && SharedPreUtil.getLoginInfo().callno_upload == 0) {
				isGuideShow1 = true;
				isGuideShow2 = true;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			vh.newsRelayContentTitle = (TextView) convertView.findViewById(R.id.newsRelayContentTitle);
			vh.newsRelayMarriage = (TextView) convertView.findViewById(R.id.newsRelayMarriage);

			// 引导上传通讯录
			vh.list_recommendContactLayer = (RelativeLayout) convertView.findViewById(R.id.list_recommendContactLayer);
			vh.list_warmLayerCloseButton = (ImageButton) convertView.findViewById(R.id.list_warmLayerCloseButton);
			vh.list_noFriendWarmInfo = (TextView) convertView.findViewById(R.id.list_noFriendWarmInfo);
		...
		// 提示是否显示通讯录
		if (position == 5 && SharedPreUtil.getLoginInfo().callno_upload == 0 && isGuideShow1) {
			// 显示通讯录
			vh.list_recommendContactLayer.setVisibility(View.VISIBLE);
			vh.list_noFriendWarmInfo.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					// 弹出导入通讯录的弹层
					mHandler.sendEmptyMessage(0x10002);
				}
			});
			vh.list_warmLayerCloseButton.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					// 关闭引导层
					isGuideShow1 = false;
		...
		} else if (position == 15 && SharedPreUtil.getLoginInfo().callno_upload == 0 && isGuideShow2) {
			// 显示通讯录
			vh.list_recommendContactLayer.setVisibility(View.VISIBLE);
			vh.list_noFriendWarmInfo.setOnClickListener(new OnClickListener() {
				@Override
		...
		} else if (position == 25 && SharedPreUtil.getLoginInfo().callno_upload == 0 && isGuideShow3) {
			// 显示通讯录
			vh.list_recommendContactLayer.setVisibility(View.VISIBLE);
			vh.list_noFriendWarmInfo.setOnClickListener(new OnClickListener() {
				@Override
			...
			vh.list_recommendContactLayer.setVisibility(View.GONE);
		}
		...
		if (msg.sex == 0) {
			// 女
			vh.newsUserNick.setTextColor(mContext.getResources().getColor(R.color.pink_color));
			vh.newsTitleInfo.setCompoundDrawablesWithIntrinsicBounds(R.drawable.icon_famale_s, 0, 0, 0);
		} else {
			// 男
			vh.newsUserNick.setTextColor(mContext.getResources().getColor(R.color.blue_main));
			vh.newsTitleInfo.setCompoundDrawablesWithIntrinsicBounds(R.drawable.icon_male_s, 0, 0, 0);
		}
		// 时间
		...
		if (msg.type == 100) {
			...
			vh.newsRelayContentTitle.setVisibility(View.GONE);
			// 标题
			if (msg.marriage == 1) {
			...
			vh.newsContentTypePhoto.setVisibility(View.GONE);
			// 显示转播模块
			vh.newsRelayContentTitle.setVisibility(View.VISIBLE);
			...
			if (msg.relayStatus == -1) {
				...
				vh.newsRelayMarriage.setVisibility(View.GONE);
				...
				vh.newsRelayMarriage.setVisibility(View.VISIBLE);
				// 昵称
				vh.newsRelayNick.setText(msg.relayNick);
				// 性别
				if (msg.relaySex == 0) {
					// 女
					vh.newsRelayNick.setTextColor(mContext.getResources().getColor(R.color.pink_color));
					vh.newsRelayMarriage.setCompoundDrawablesWithIntrinsicBounds(R.drawable.icon_famale_s, 0, 0, 0);
				} else {
					// 男
					vh.newsRelayNick.setTextColor(mContext.getResources().getColor(R.color.blue_main));
					vh.newsRelayMarriage.setCompoundDrawablesWithIntrinsicBounds(R.drawable.icon_male_s, 0, 0, 0);
				}
				// 婚姻状态
				vh.newsRelayMarriage.setVisibility(View.VISIBLE);
				if (msg.relayMarriage == 1) {
					vh.newsRelayMarriage.setText("单身");
				} else if (msg.relayMarriage == 2) {
					vh.newsRelayMarriage.setText("恋爱中");
				} else if (msg.relayMarriage == 3) {
					vh.newsRelayMarriage.setText("已婚");
				} else {
					vh.newsRelayMarriage.setVisibility(View.GONE);
				}
				// 日期
	...
	private class ViewHolder {
		...
		public TextView newsRelayContentTitle;// 转播动态的标题“转播动态”
		public TextView newsRelayMarriage;// 被转播动态的人的婚姻状态

		// 引导上传通讯录
		public RelativeLayout list_recommendContactLayer;
		public ImageButton list_warmLayerCloseButton;
		public TextView list_noFriendWarmInfo;
	}
}
