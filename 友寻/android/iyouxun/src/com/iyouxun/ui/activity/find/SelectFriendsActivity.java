		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			if (bean.isChecked()) {
				...
				for (int i = 0; i < selectdata.size(); i++) {
					if (bean.getUid().equals(selectdata.get(i).getUid())) {
						selectdata.remove(i);
						break;
					}
				}
			...
			for (int j = 0; j < selectdata.size(); j++) {
				...
				J_NetManager.getInstance().loadIMG(null, selectdata.get(j).getAvatar(), avatar, 0, 0);
				selectedLinearLayout.addView(item_view);
			}
			if (selectdata.size() > 0) {
				btnOk.setText("确定（" + selectdata.size() + "）");
			} else {
