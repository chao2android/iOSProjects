...
public class NewsMainActivity extends CommTitleActivity {
	...
	/** 发布动态返回 */
	private final int REQUEST_CODE_CREATE = 0X4001;
	public static final int RESULT_CODE_CREATE_OK = 0X4001;
	...
	protected void initViews() {
		...
		adapter = new NewsListInfoAdapter(mContext, CURRNET_UID);
			...
			if (J_Cache.sLoginUser.friends_num > 0) {
				...
				if (J_Cache.sLoginUser.marriage != 1) {
					...
					if (J_Cache.sLoginUser.callno_upload == 0) {
						...
						String eStr = "你还没有好友，赶快<font color=\"#2695FF\">导入通讯录</font>，<br />看看好友在干什么";
						newsMainEmptyLayer.setText(Html.fromHtml(eStr));
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		} else if (requestCode == REQUEST_CODE_CREATE && resultCode == RESULT_CODE_CREATE_OK) {
			// 隐藏键盘
			hideKeyboard();
			// 发布动态返回
			receiveRefresh();
			...
			for (int i = 0; i < recommendFriends.size(); i++) {
				...
				recommend_friend_avatar.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						profileIntent.putExtra("userType", 1);
	...
	private void parseNewsInfo(String newsInfo, String type, boolean isCache) {
		try {
			...
			if (PAGE_DY_ID == 0) {
				adapter.setGuideShowStatus(true);

				datas = new ArrayList<NewsInfoBean>();
				// 缓存数据
			...
			if (dataArray.length() > 0) {
				...
				for (int i = 0; i < dataArray.length(); i++) {
					...
					if (bean.friendDimen == 2 && SharedPreUtil.getLoginInfo().show_second_friend_dync == 0 && CURRNET_UID == 0) {
						// 如果用户设置了不在熟人圈显示二度好友动态，并且当前用户为二度好友
						continue;
					}
					// 赞列表
	...
	private void showNewsPopMenu(View v) {
		new MenuPopDialog(mContext).setDefault(is_look_single).setCallBack(new OnSelectCallBack() {
			@Override
			public void onCallBack(String value1, String value2, String value3) {
				if (value1.equals("1")) {
					// 查看全部
	...
	private void gotoCreateNewsPage() {
		...
		startActivityForResult(newsIntent, REQUEST_CODE_CREATE);
	}
