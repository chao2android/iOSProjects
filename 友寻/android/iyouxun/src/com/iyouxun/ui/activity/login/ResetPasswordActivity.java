...
public class ResetPasswordActivity extends CommTitleActivity {
	...
	private final TextWatcher textWatcher = new TextWatcher() {
		...
		public void afterTextChanged(Editable s) {
			...
			if (Util.isMobileNumber(mobile) && securityCode.length() > 0 && newPassword.length() > 0
					&& confirmPassword.length() > 0) {// 验证手机号
	...
	private final OnClickListener listener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.reset_password_btn_submit:// 提交按钮
				...
				if (!passwordAgain.equals(passwordString)) {
					ToastUtil.showToast(mContext, "两次输入的密码不一致");
					return;
				} else if (passwordAgain.length() < 6) {
					ToastUtil.showToast(mContext, getString(R.string.password_length_error));
					return;
				}
				DialogUtils.showProgressDialog(mContext, LoadingHandler.DEFALT_STR);
