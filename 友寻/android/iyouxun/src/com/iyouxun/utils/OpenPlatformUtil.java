...
public class OpenPlatformUtil {
	...
	public static J_ShareParams getShareFriendParams(LoginUser currentUserInfo) {
		...
		String sexOtherInfo = currentUserInfo.sex == 0 ? "真命天女" : "真命天子";
		String shareInfo = "";
		if (currentUserInfo.marriage == 1) {
			// 单身状态
			StringBuilder profileSb = new StringBuilder(sexInfo + "的资料：" + currentUserInfo.nickName);
			if (currentUserInfo.star > 0) {
				profileSb.append("," + ProfileUtils.getStar(currentUserInfo.star));// 星座
			}
			if (currentUserInfo.birthpet > 0) {
				profileSb.append(",属" + ProfileUtils.getBirthPet(currentUserInfo.birthpet));// 属相
			}
			if (!Util.isBlankString(currentUserInfo.locationName) && !Util.isBlankString(currentUserInfo.subLocationName)) {
				profileSb.append("," + currentUserInfo.locationName + currentUserInfo.subLocationName);
			}
			if (currentUserInfo.lonelyConfirm > 0) {
				profileSb.append("," + currentUserInfo.lonelyConfirm + "位好友认证" + sexInfo + "是单身");
			}
			shareInfo = "我是" + currentUserInfo.nickName + "的朋友，" + sexInfo + "已经单身好久啦，你觉得" + sexInfo + "是否会是你的" + sexOtherInfo
					+ "呢？" + profileSb.toString() + ",详细资料：" + url;
		} else {
			// 非单身状态
