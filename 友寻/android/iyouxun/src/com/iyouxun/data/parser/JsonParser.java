...
public class JsonParser extends J_JsonParser {
	...
	public Object parsJson(String jsonStr, J_Request request) throws Exception {
		...
		case NetTaskIDs.TASKID_UPDATE_GROUP_SHOW:
			// 修改用户群在资料页显示状态
		case NetTaskIDs.TASKID_IS_GROUP_MEMBER:
			// 是否是群组成员
		case NetTaskIDs.TASKID_UPDATE_USER_GROUP_SHOW_NICK:
			// 是否展示群聊昵称
		case NetTaskIDs.TASKID_SET_USER_VOICE_SWITCH:
			// 设置听筒模式和扬声器模式开关
		case NetTaskIDs.TASKID_GET_USER_VOICE_SWITCH:
			// 获取听筒模式和扬声器模式开关
		case NetTaskIDs.TASKID_SEND_SINGLE_MSG:
			// 单聊求脱单
		case NetTaskIDs.TASKID_SEND_GROUP_SINGLE_MSG:
			// 群聊求脱单
		case NetTaskIDs.TASKID_ALTERNATIVE_TAG_LIST:
			// 获取推荐标签列表
		case NetTaskIDs.TASKID_IS_EXIST_PHOTO:
			// 生活照是否存在
		case NetTaskIDs.TASKID_PRAISE_PHOTO:
			// 赞照片
		case NetTaskIDs.TASKID_PRAISE_PHOTO_CANCLE:
			// 取消赞照片
	...
	private UserInfoResponse parseUserInfo(String str) throws Exception {
		...
		if (jsonObject.optInt("retcode") == 1) {
			...
			// gid
			parseUser.gid = userInfos.optInt("gid");
			// 昵称
